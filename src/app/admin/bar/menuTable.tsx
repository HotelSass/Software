...
const MenuTable = ({ data, category }: any) => {
  ...
  async function deleteCategory(title: string) {
    try {
      const response = await fetch(serverUrl + "/bar/deleteItem/"+title.toLowerCase(), {
        method: 'DELETE',
        headers: {
        ...
        <tbody>
          {data.map((item: any, index: number) => (
            <tr key={index} className="bg-gray-100 border-b h-12">
              ...
              <td className="p-2 text-ssm pl-3 capitalize">{item.itemName}</td>
              <td className="p-2 text-ssm pl-3 capitalize">{item.category}</td>
              <td className="p-2 pl-3 w-40 text-[10px] text-ellipsis">
                Rs. {item.price || item.distribution.full}
              </td>
